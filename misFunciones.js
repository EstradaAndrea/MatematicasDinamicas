...
function Biseccion(f, a, b, n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    salida += `Iteración ${i + 1}. x = ${medio.toFixed(6)}, f(x) = ${f(
      medio
    ).toFixed(6)}<br>`;
...
function dibujarLineaPoligonal(event) {
  ...
  document.getElementById("coordenadas").innerText = `(${x.toFixed(
    1
  )}, ${y.toFixed(1)})`;
...
function inicializarInterseccionRectas() {
  ...
  canvas.addEventListener("click", (e) => {
    ...
    if (rectas.length === 2) {
      const mPoints = rectas
        .flat()
        .map((p) => coordCanvasToMath(p.x, p.y, centro, escala));
      ...
      if (intersection) {
        ...
        document.getElementById(
          "resultado"
        ).innerText = `Intersección: (${intersection.x.toFixed(
          2
...
function dibujarRectasDesdeFormula() {
  ...
  drawPoint(ctx, p, "red");
  document.getElementById("resultado").innerText = `Intersección: (${x.toFixed(
    2
  )}, ${y.toFixed(2)})`;
}

// ===================== Ejercicio 6.1: Suma geométrica =====================
...
function agregarFraccion() {
  ...
  nivel++;
  document.getElementById("suma").innerText = `Suma actual: ${suma.toFixed(
    10
  )}`;
}
